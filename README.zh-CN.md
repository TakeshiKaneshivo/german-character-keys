...
1. macOS：打开 DMG，将 `German Character Keys.app` 拖入 `Applications` 文件夹；Windows：运行安装程序，然后启动应用。
2. 在主界面开启德语字符映射。
3. 在支持输入的文本框中按下上述按键。
...
手动启动应用时，主窗口会立即显示。关闭主窗口只会将应用隐藏到 macOS 菜单栏或 Windows 系统托盘，不会退出应用；点击托盘图标即可重新打开。启用“登录时自动启动”后，应用会在登录时静默驻留在菜单栏或系统托盘中。
