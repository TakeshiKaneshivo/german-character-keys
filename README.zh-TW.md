...
1. macOS：開啟 DMG，將 `German Character Keys.app` 拖入 `Applications` 資料夾；Windows：執行安裝程式，然後啟動應用程式。
2. 在主介面啟用德語字元映射。
3. 在支援輸入的文字欄位中按下上述按鍵。
...
手動啟動應用程式時，主視窗會立即顯示。關閉主視窗只會將應用程式隱藏到 macOS 選單列或 Windows 系統匣，不會退出應用程式；點擊系統匣圖示即可重新開啟。啟用「登入時自動啟動」後，應用程式會在登入時安靜地駐留在選單列或系統匣中。
