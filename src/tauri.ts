...
const previewDefaultShortcut = typeof navigator !== "undefined" && /Mac|iPhone|iPad/i.test(navigator.platform) ? "Command+K" : "Ctrl+K";
let previewStatus: Status = { enabled: false, shortcut_registered: true, accessibility_granted: true, shortcut: previewDefaultShortcut, platform: "preview", launch_at_login: false, backend_status: "disabled", message: null };
...
export const resetShortcut = () => runningInTauri ? invoke<OperationResult>("reset_shortcut") : Promise.resolve((previewStatus = { ...previewStatus, shortcut: previewDefaultShortcut }, previewResult()));
