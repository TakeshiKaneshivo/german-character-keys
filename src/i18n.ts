...
export type TranslationKey =
  | "brandEyebrow" | "appTitle" | "appSubtitle" | "appSubtitleFull" | "languageMenuLabel" | "themeMenuLabel" | "themeSystem" | "themeLight" | "themeDark" | "closeHelp" | "moreMenuLabel" | "helpMenuItem" | "toggleMappingLabel" | "shortcutAriaLabel"
  ...
  | "helpKeyboardCaption" | "helpMappingKicker" | "helpMappingTitle" | "helpShiftUpper" | "helpNote" | "backToMain"
...
export function applyTranslations(root: ParentNode = document) {
  root.querySelectorAll<HTMLElement>("[data-i18n]").forEach((element) => {
    ...
    if (key in translations[getLanguage()]) element.textContent = t(key);
  });
  ...
  root.querySelectorAll<HTMLElement>("[data-i18n-title]").forEach((element) => {
    ...
    if (key in translations[getLanguage()]) element.setAttribute("title", t(key));
  });
  document.documentElement.lang = getLanguage();
}

export function bindLanguageMenu(toggle: HTMLButtonElement, menu: HTMLElement, onChange: () => void) {
  const render = () => {
    const current = getLanguage();
    toggle.textContent = languageAbbreviation(current);
    toggle.setAttribute("aria-label", t("languageMenuLabel"));
    menu.querySelectorAll<HTMLButtonElement>("[data-language]").forEach((item) => {
      const selected = item.dataset.language === current;
  ...
  menu.querySelectorAll<HTMLButtonElement>("[data-language]").forEach((item) => item.addEventListener("click", () => {
    const language = item.dataset.language as Language;
    if (!LANGUAGE_OPTIONS.some((option) => option.code === language)) return;
